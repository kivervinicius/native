...
[[ ! -z $IOS_CERTIFICATE_CER ]] && \
  security import $IOS_CERTIFICATE_CER -k $keychain_file -T /usr/bin/codesign

[[ ! -z $IOS_CERTIFICATE_P12 ]] && \
  security import $IOS_CERTIFICATE_P12 -k $keychain_file -P "$IOS_CERTIFICATE_PASSWORD" -T /usr/bin/codesign
